				   ...
				   unsigned long event, void *data)
{
	...
	if (event !=
	    MSM_DRM_EARLY_EVENT_BLANK /*FB_EVENT_BLANK FB_EARLY_EVENT_BLANK */ )
	{
		return 0;
	}
...
#ifdef FEATURE_NUBIA_FINGERPRINT_AUTODETECT
	auto_dev = &sf_ctl_dev;
#define NUBIA_SENSORS_FP_ID_GPIO 125
	/*auto_dev->vdd_reg = regulator_get(&auto_dev->pdev->dev, SF_VDD_NAME);
	   if (IS_ERR(auto_dev->vdd_reg)) {
	   err = PTR_ERR(auto_dev->vdd_reg);
	   xprintk(KERN_ERR, "Regulator get failed vdd err = %d\n", err);
	   return err;
	   }

	   xprintk(KERN_INFO, "[%s] line=%d\n",__func__, __LINE__);
	   err = gpio_request(NUBIA_SENSORS_FP_ID_GPIO, "NUBIA_ID_PIN");
	   if (err < 0) {
	   xprintk(KERN_INFO, "[%s] Failed to request GPIO=%d, err =%d",__func__, NUBIA_SENSORS_FP_ID_GPIO, err);
	   return err;
	   }
	   xprintk(KERN_INFO, "[%s] line=%d\n",__func__, __LINE__);
	   if (regulator_count_voltages(auto_dev->vdd_reg) > 0) {
	   err = regulator_set_voltage(auto_dev->vdd_reg, SF_VDD_MIN_UV, SF_VDD_MAX_UV);
	   if (err) {
	   ...
	   xprintk(KERN_INFO, "[%s] line=%d\n",__func__, __LINE__);
	   auto_dev->power_on(true);
	   mdelay(5); */
	id_gpio = gpio_get_value(NUBIA_SENSORS_FP_ID_GPIO);
	if (id_gpio) {
		xprintk(KERN_INFO,
			"id_gpio = %d. It's fingerprint chip of vendor : sunwave\n",
			id_gpio);
		gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
		//auto_dev->power_on(false);
		//sf_ctl_dev.free_gpio(&sf_ctl_dev);
	} else {
		xprintk(KERN_ERR,
			"id_gpio = %d. It's fingerprint chip of vendor : silead, now driver will exit\n",
			id_gpio);
		gpio_free(NUBIA_SENSORS_FP_ID_GPIO);
		auto_dev->power_on(false);
		sf_ctl_dev.free_gpio(&sf_ctl_dev);
...
static int sf_ctl_init_input(void)
{
	...
	__set_bit(KEY_CAMERA, sf_ctl_dev.input->keybit);
...
static int __init sf_ctl_driver_init(void)
{
	...
	if (err == (-ENODEV)) {
		xprintk(KERN_ERR,
			"%s, Failed to register platform driver. err == ENODEV\n",
			__FUNCTION__);
		return -ENODEV;
	}
#endif
