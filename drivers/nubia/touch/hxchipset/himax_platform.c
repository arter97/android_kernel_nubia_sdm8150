		   ...
		   struct himax_i2c_platform_data *pdata)
{
	...
	if (prop) {
		...
		if (coords_size != 4)
			D(" %s:Invalid panel coords size %d", __func__,
			  coords_size);
	}

	if (of_property_read_u32_array
	    (dt, "himax,panel-coords", coords, coords_size) == 0) {
	...
	if (prop) {
		coords_size = prop->length / sizeof(u32);
		if (coords_size != 4)
			D(" %s:Invalid display coords size %d", __func__,
			  coords_size);
	}
	...
	I(" DT:gpio_irq=%d, gpio_rst=%d, gpio_3v3_en=%d", pdata->gpio_irq,
