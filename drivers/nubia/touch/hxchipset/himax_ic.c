...
int himax_write_read_reg(struct i2c_client *client, uint8_t * tmp_addr,
...
uint32_t himax_get_rawdata(struct i2c_client * client, uint32_t RAW[],
			   uint32_t datalen)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
	...
	for (i = 0; i < ic_data->HX_TX_NUM * ic_data->HX_RX_NUM; i++) {
		RAW[i] =
		    tmp_rawdata[(i * 2) + 1 + 4] * 256 + tmp_rawdata[(i * 2) +
								     4];
	}
#if 1
...
void himax_set_N_frame(struct i2c_client *client, uint16_t Nframe,
		       uint8_t checktype)
{
	uint8_t tmp_addr[4];
	uint8_t tmp_data[4];
...
uint32_t mpTestFunc(struct i2c_client * client, uint8_t checktype,
		    uint32_t datalen)
{
	uint32_t i /*, j */ , ret = 0;
...
int himax_chip_self_test(struct i2c_client *client)
{
	...
	I("[MP_OPEN_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_OPEN,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("1. Open Test: End %d\n\n\n", ret);
	I("[MP_MICRO_OPEN_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_MICRO_OPEN,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("2. Micro Open Test: End %d\n\n\n", ret);
	I("[MP_SHORT_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_SHORT,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	...
	I("[MP_RAW_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_RAWDATA,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("4. Short Test: End %d\n\n\n", ret);
	I("[MP_NOISE_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_NOISE,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	...
	I("[MP_DOZE_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_DOZE_RAWDATA,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("6. MP_DOZE_TEST_RAW: End %d\n\n\n", ret);
	I("[MP_DOZE_TEST_NOISE]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_DOZE_NOISE,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	...
	I("[MP_LPWUG_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_LPWUG_RAWDATA,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("8. MP_LPWUG_TEST_RAW: End %d\n\n\n", ret);
	I("[MP_LPWUG_TEST_NOISE]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_LPWUG_NOISE,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("9. MP_LPWUG_TEST_NOISE: End %d\n\n\n", ret);
	I("[MP_LPWUG_IDLE_TEST_RAW]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_LPWUG_IDLE_RAWDATA,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
	I("10. MP_LPWUG_IDLE_TEST_RAW: End %d\n\n\n", ret);
	I("[MP_LPWUG_IDLE_TEST_NOISE]\n");
	ret =
	    mpTestFunc(client, HIMAX_INSPECTION_LPWUG_IDLE_NOISE,
		       (ic_data->HX_TX_NUM * ic_data->HX_RX_NUM) +
		       ic_data->HX_TX_NUM + ic_data->HX_RX_NUM);
			 ...
			 int read_length, uint8_t * read_data, bool cfg_flag)
{
	...
	if (cfg_flag == false) {
		...
		if (i2c_himax_write
		    (client, 0x00, tmp_data, 4, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
		tmp_data[0] = 0x00;
		if (i2c_himax_write
		    (client, 0x0C, tmp_data, 1, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}

		if (i2c_himax_read
		    (client, 0x08, read_data, read_length,
		     DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
		...
		if (i2c_himax_read
		    (client, read_addr[0], read_data, read_length,
		     DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
...
void himax_flash_write_burst_lenth(struct i2c_client *client,
				   ...
				   int length)
{
	...
	if (i2c_himax_write
	    (client, 0x00, data_byte, length + 4, DEFAULT_RETRY_CNT) < 0) {
		E("%s: i2c access fail!\n", __func__);
		return;
	}
}
			  ...
			  int write_length, uint8_t * write_data, bool cfg_flag)
{
	...
	if (cfg_flag == false) {
		address =
		    (write_addr[3] << 24) + (write_addr[2] << 16) +
		...
		for (i = address; i < address + write_length; i++) {
			...
			himax_flash_write_burst_lenth(client, write_addr,
						      write_data, write_length);
		}
	} else if (cfg_flag == true) {
		if (i2c_himax_write
		    (client, write_addr[0], write_data, write_length,
		     DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
...
bool himax_sense_off(struct i2c_client * client)
{
	...
	do {
		...
		tmp_data[0] = 0x27;
		if (i2c_himax_write
		    (client, 0x31, tmp_data, 1, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return false;
		}
		//===========================================
		...
		tmp_data[0] = 0x95;
		if (i2c_himax_write
		    (client, 0x32, tmp_data, 1, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return false;
		}
		// ======================
...
void himax_interface_on(struct i2c_client *client)
{
	...
	do {
		...
		tmp_data[0] = 0x31;
		if (i2c_himax_write
		    (client, 0x13, tmp_data, 1, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
		//===========================================
		...
		tmp_data[0] = (0x10);
		if (i2c_himax_write
		    (client, 0x0D, tmp_data, 1, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
		// Check cmd
...
void himax_sense_on(struct i2c_client *client, uint8_t FlashMode)
{
	...
	if (!FlashMode) {
		...
		do {
			...
			tmp_data[1] = 0x00;
			tmp_data[0] = 0x53;
			himax_register_write(client, tmp_addr, 4, tmp_data,
					     false);

			tmp_addr[0] = 0xE4;
			himax_register_read(client, tmp_addr, 4, tmp_data,
					    false);
		...
		if (retry >= 5) {
			...
			tmp_data[0] = 0x00;
			if (i2c_himax_write
			    (client, 0x31, tmp_data, 1,
			     DEFAULT_RETRY_CNT) < 0) {
				E("%s: i2c access fail!\n", __func__);
			}
			if (i2c_himax_write
			    (client, 0x32, tmp_data, 1,
			     DEFAULT_RETRY_CNT) < 0) {
				E("%s: i2c access fail!\n", __func__);
			}
			     ...
			     int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size;
		...
		if (i2c_himax_write
		    (client, 0x00, buring_data, 20, DEFAULT_RETRY_CNT) < 0) {
			E("%s: i2c access fail!\n", __func__);
			return;
		}
		//=================================
		...
		for (j = 0; j < 5; j++) {
			...
			if (i2c_himax_write
			    (client, 0x00, buring_data, program_length + 4,
			     DEFAULT_RETRY_CNT) < 0) {
				E("%s: i2c access fail!\n", __func__);
				return;
			}
...
uint32_t himax_hw_check_CRC(struct i2c_client * client, uint8_t * start_addr,
			    int reload_length)
{
	uint32_t result = 0;
	uint8_t tmp_addr[4];
...
void himax_flash_page_write(struct i2c_client *client, uint8_t * write_addr,
			    uint8_t * write_data)
{
...
// Product ID
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11)
		    && ((tmp_data[1] == 0x2a) || (tmp_data[1] == 0x2e))) {
...
#ifdef HX_FIX_NEW_PANEL
			g_i_new_FW_VER =
			    i_CTPM_FW_NEW[FW_VER_MAJ_FLASH_ADDR] << 8 |
			    i_CTPM_FW_NEW[FW_VER_MIN_FLASH_ADDR];
			g_i_new_CFG_VER =
			    i_CTPM_FW_NEW[CFG_VER_MAJ_FLASH_ADDR] << 8 |
			    i_CTPM_FW_NEW[CFG_VER_MIN_FLASH_ADDR];
...
void himax_get_DSRAM_data(struct i2c_client *client, uint8_t * info_data)
{
	...
	tmp_data[0] = 0xA5;
	fw_run_flag =
	    himax_write_read_reg(client, tmp_addr, tmp_data, 0xA5, 0x5A);
...
bool diag_check_sum(struct himax_report_data * hx_touch_data)	//return checksum value
{
	...
	for (i = 0, check_sum_cal = 0;
	     ...
	     i = i + 2) {
		check_sum_cal +=
		    (hx_touch_data->hx_rawdata_buf[i + 1] * 256 +
...
uint8_t himax_read_DD_status(uint8_t * cmd_set, uint8_t * tmp_data)
{
	...
	{
		himax_register_read(private_ts->client, cmd_addr, 4, tmp_data,
				    false);
	...
	himax_register_read(private_ts->client, tmp_addr, req_size, tmp_data,
			    false);
	return NO_ERR;
}
...
int himax_read_FW_status(uint8_t * state_addr, uint8_t * tmp_addr)
{
	...
	if (state_addr[0] == 0x01) {
		...
		req_size = 0x04;
		himax_sense_off(private_ts->client);
		himax_register_read(private_ts->client, status_addr, req_size,
				    tmp_addr, false);
		...
		req_size = 0x30;
		himax_register_read(private_ts->client, cmd_addr, req_size,
				    tmp_addr, false);
	}
	...
	for (custom_info_temp = 0; custom_info_temp < HX_GUEST_INFO_SIZE;
		...
		info_len = flash_tmp_buffer[custom_info_temp * flash_page_len]
		    ...
		    (flash_tmp_buffer[custom_info_temp * flash_page_len + 3] <<
		     24);
		I("Now custom_info_temp = %d,[0]=%d,[1]=%d,[2]=%d,[3]=%d,info_len=%d\n", custom_info_temp, flash_tmp_buffer[custom_info_temp * flash_page_len]
		  , flash_tmp_buffer[custom_info_temp * flash_page_len + 1]
		  , flash_tmp_buffer[custom_info_temp * flash_page_len + 2]
		  , flash_tmp_buffer[custom_info_temp * flash_page_len + 3]
		  , info_len);
		...
		if (custom_info_temp == 0) {
			for (i = 0; i < 128; i++) {
				if (i % 16 == 0 && i > 0)
					I("\n");
				I("g_guest_str[%d][%d] = 0x%02X",
				  custom_info_temp, i,
				  g_guest_str[custom_info_temp][i]);
			}
		}
	}
...
void himax_clean_sram_0f(uint8_t * addr, int write_len, int type)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		...
		if (total_size_temp >= max_bus_size) {
			himax_flash_write_burst_lenth(private_ts->client,
						      tmp_addr, tmp_data,
						      max_bus_size);
...
void himax_write_sram_0f(const struct firmware *fw_entry, uint8_t * addr,
			 int start_index, int write_len)
{
	int total_read_times = 0;
	int max_bus_size = 128;
	int total_size_temp = 0;
...
#ifdef HX_0F_DEBUG
void himax_read_sram_0f(const struct firmware *fw_entry, uint8_t * addr,
			int start_index, int read_len)
{
	...
	for (i = 0; i < (total_read_times); i++) {
		if (total_size_temp >= max_i2c_size) {
			himax_register_read(private_ts->client, tmp_addr,
					    max_i2c_size,
					    &temp_info_data[i * max_i2c_size],
					    false);
			total_size_temp = total_size_temp - max_i2c_size;
