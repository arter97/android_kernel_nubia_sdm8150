...
int hx83112b_jitter_switch(bool on)
{
	...
	if (on) {		//jitter off : enter game mode
		do {
			...
			tmp_data[1] = 0xA5;
			tmp_data[0] = 0x5A;
			himax_flash_write_burst(private_ts->client, tmp_addr,
						tmp_data);

			himax_register_read(private_ts->client, tmp_addr, 4,
					    tmp_data, false);
				 ...
				 size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "%s \n",
			     HIMAX_DRIVER_VER);
				 ...
				 size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "%d ", ts->irq_enabled);
		ret += snprintf(temp_buf + ret, len - ret, "\n");
		if (copy_to_user(buf, temp_buf, len))
			I("%s,here:%d\n", __func__, __LINE__);
		kfree(temp_buf);
				 ...
				 size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "%d ",
			     ts->pdata->abs_x_min);
		ret +=
		    snprintf(temp_buf + ret, len - ret, "%d ",
			     ts->pdata->abs_x_max);
					...
					size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		for (loop_i = 0; loop_i < 128; loop_i++) {
			ret +=
			    snprintf(temp_buf + ret, len - ret, "0x%2.2X ",
				     data[loop_i]);
			if ((loop_i % 16) == 15)
				ret +=
				    snprintf(temp_buf + ret, len - ret, "\n");
...
{
	diag_mutual_2 =
	    kzalloc(x_channel_2 * y_channel_2 * sizeof(int32_t), GFP_KERNEL);
}
#endif
...
void himax_log_touch_data(uint8_t * buf,
			  struct himax_report_data *hx_touch_data)
{
	int loop_i = 0;
	int print_size = 0;
				  ...
				  int touched, uint16_t old_finger)
{
	...
	if (touched == HX_FINGER_ON) {
		if (old_finger >> loop_i == 0) {
			if (ts->useScreenRes) {
				I("status: Screen:F:%02d Down, X:%d, Y:%d, W:%d, N:%d\n", loop_i + 1, x * ts->widthFactor >> SHIFTBITS, y * ts->heightFactor >> SHIFTBITS, w, EN_NoiseFilter);
			} else {
	...
	} else if (touched == HX_FINGER_LEAVE)	//if (old_finger >> loop_i == 1)
	{
		if (old_finger >> loop_i == 1) {
			if (ts->useScreenRes) {
				I("status: Screen:F:%02d Up, X:%d, Y:%d, N:%d\n", loop_i + 1, ts->pre_finger_data[loop_i][0] * ts->widthFactor >> SHIFTBITS, ts->pre_finger_data[loop_i][1] * ts->heightFactor >> SHIFTBITS, EN_NoiseFilter);	//Last_EN_NoiseFilter
			} else {
				I("status: Raw:F:%02d Up, X:%d, Y:%d, N:%d\n", loop_i + 1, ts->pre_finger_data[loop_i][0], ts->pre_finger_data[loop_i][1], EN_NoiseFilter);	//Last_EN_NoiseFilter
			}
		}
	} else {
		I("%s : wrong input!\n", __func__);
...
static void nubia_diag_write(int num)
{
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (storage_type == 4) {
			...
			case 1:
				diag_sram_fn =
				    filp_open(IIR_DUMP_FILE, O_CREAT | O_WRONLY,
					      0);
				break;
			case 2:
				diag_sram_fn =
				    filp_open(DC_DUMP_FILE, O_CREAT | O_WRONLY,
					      0);
				break;
			case 3:
				diag_sram_fn =
				    filp_open(BANK_DUMP_FILE,
					      O_CREAT | O_WRONLY, 0);
				break;
			default:
...
static void *nubia_BASEDC_data_seq_next(struct seq_file *s, void *v,
					loff_t * pos)
{
	return NULL;
}
				...
				size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (debug_level_cmd == 't') {
			...
			ret +=
			    snprintf(temp_buf + ret, len - ret, "RX Num : %d\n",
				     ic_data->HX_RX_NUM);
			ret +=
			    snprintf(temp_buf + ret, len - ret, "TX Num : %d\n",
				     ic_data->HX_TX_NUM);
			ret +=
			    snprintf(temp_buf + ret, len - ret, "BT Num : %d\n",
				     ic_data->HX_BT_NUM);
			ret +=
			    snprintf(temp_buf + ret, len - ret,
				     "X Resolution : %d\n", ic_data->HX_X_RES);
			...
			else if (himax_read_ic_trigger_type(private_ts->client)
				 == 0)
				ret +=
				    snprintf(temp_buf + ret, len - ret,
...
static void *nubia_user_update_seq_next(struct seq_file *s, void *v,
					loff_t * pos)
{
	return NULL;
}
...
static int nubia_request_firmware(const char *name, long *size, char **data)
{
	...
	retval =
	    security_kernel_post_read_file(file_loadimg, img_data, img_size,
					   id);
	if (retval) {
				     ...
				     size_t len, loff_t * pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		ret +=
		    snprintf(temp_buf + ret, len - ret, "FlashStart:%2.2x \n",
...
int himax_touch_proc_init(void)
{
...
#ifdef NUBIA_TP_HIMAX_PROC_IC_DETECT
	nubia_proc_ic_detect_file =
	    proc_create(NUBIA_PROC_IC_DETECT_FILE, (S_IRUGO),
...
#ifdef NUBIA_TP_HIMAX_PROC_DC_DATA
	nubia_DC_data_file =
	    proc_create(NUBIA_DC_DATA_FILE, (S_IWUSR | S_IRUGO),
...
#ifdef NUBIA_TP_HIMAX_PROC_IIR_DATA
	nubia_IIR_data_file =
	    proc_create(NUBIA_IIR_DATA_FILE, (S_IWUSR | S_IRUGO),
...
#endif
#ifdef NUBIA_TP_HIMAX_PROC_SMART_WAKEUP
	nubia_proc_smart_wakeup_file =
	    proc_create(NUBIA_PROC_SMART_WAKEUP_FILE, (S_IRUGO),
...
#endif
#ifdef NUBIA_TP_HIMAX_PROC_USER_UPDATE
	nubia_user_update_file =
	    proc_create(NUBIA_USER_UPDATE_FILE, (S_IWUSR | S_IRUGO),
...
#endif
#ifdef NUBIA_TP_HIMAX_PROC_BASEDC_DATA
	nubia_BASEDC_data_file =
	    proc_create(NUBIA_BASEDC_DATA_FILE, (S_IWUSR | S_IRUGO),
	...
	for (attr_count = 0; attr_count < ARRAY_SIZE(nubia_tp_attrs);
	     attr_count++) {
		retval =
		    sysfs_create_file(ts->nubia_tp_kobj,
				      &nubia_tp_attrs[attr_count].attr);
		if (retval < 0) {
 ...
 err_sys_creat:
	for (--attr_count; attr_count >= 0; attr_count--)
		sysfs_remove_file(ts->nubia_tp_kobj,
				  &nubia_tp_attrs[attr_count].attr);
	ts = NULL;
#ifdef NUBIA_TP_HIMAX_PROC_BASEDC_DATA
 fail_27:
