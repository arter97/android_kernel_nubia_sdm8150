...
static void calcDataSize(uint8_t finger_num)
{
	...
	ts_data->raw_data_frame_size =
	    128 - ts_data->coord_data_size - ts_data->area_data_size - 4 - 4 -
	    1;
...
static int i_update_FW(void)
{
 ...
 update_retry:
	if (fullFileLength == FW_SIZE_32k) {
		...
		ret =
		    fts_ctpm_fw_upgrade_with_sys_fs_60k(private_ts->client,
							ImageBuffer,
							fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_64k) {
#ifdef HX_FIX_NEW_PANEL
		if (ic_data->vendor_touch_cfg_ver < 0x30
		    && ic_data->vendor_touch_cfg_ver != 0) {
			ret =
			    fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->
								client,
								ImageBuffer,
								fullFileLength,
								false);
...
#else
		ret =
		    fts_ctpm_fw_upgrade_with_sys_fs_64k(private_ts->client,
							ImageBuffer,
							fullFileLength, false);
#endif
	} else if (fullFileLength == FW_SIZE_124k) {
		ret =
		    fts_ctpm_fw_upgrade_with_sys_fs_124k(private_ts->client,
							 ImageBuffer,
							 fullFileLength, false);
	} else if (fullFileLength == FW_SIZE_128k) {
		ret =
		    fts_ctpm_fw_upgrade_with_sys_fs_128k(private_ts->client,
							 ImageBuffer,
							 fullFileLength, false);
	}
	if (ret == 0) {
...
static int himax_parse_wake_event(struct himax_ts_data *ts)
{
	uint8_t *buf;
#ifdef HX_GESTURE_TRACK
	int tmp_max_x = 0x00, tmp_min_x = 0xFFFF, tmp_max_y = 0x00, tmp_min_y =
	    0xFFFF;
	int gest_len;
#endif
	...
	if (buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 &&
		...
		while (i < (gest_len + 1) / 2) {
			gest_pt_log_coordinate(buf
					       [GEST_PTLG_ID_LEN + 4 + i * 2],
...
int himax_report_data_init(void)
{
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00)	//more than 4 fingers
	{
		hx_touch_data->rawdata_size =
		    cal_data_len(hx_touch_data->raw_cnt_rmd, ic_data->HX_MAX_PT,
...
static void himax_report_points(struct himax_ts_data *ts)
{
	...
	if (hx_point_num != 0) {
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			base = loop_i * 4;
			x = hx_touch_data->
			    hx_coord_buf[base] << 8 | hx_touch_data->
			    hx_coord_buf[base + 1];
			y = (hx_touch_data->
			     hx_coord_buf[base +
					  2] << 8 | hx_touch_data->
			...
			if (x >= 0 && x <= ts->pdata->abs_x_max && y >= 0
				...
				if ((ts->debug_log_level & BIT(3)) > 0) {
					himax_log_touch_event_detail(ts, x, y,
								     w, loop_i,
								     EN_NoiseFilter,
								     HX_FINGER_ON,
								     old_finger);
	...
	/* finger leave/release */
		...
		for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++) {
			...
			if (ts->pre_finger_mask > 0
				...
				if (((ts->pre_finger_mask >> loop_i) & 1) == 1) {
					if (ts->useScreenRes) {
						I("status:%X, Screen:F:%02d Up, X:%d, Y:%d, N:%d\n", 0, loop_i + 1, ts->pre_finger_data[loop_i][0] * ts->widthFactor >> SHIFTBITS, ts->pre_finger_data[loop_i][1] * ts->heightFactor >> SHIFTBITS, Last_EN_NoiseFilter);
					} else {
...
int himax_touch_get(struct himax_ts_data *ts, uint8_t * buf, int ts_status)
{
	...
	case 1:
		...
		if ((hx_touch_data->diag_cmd)
			...
			ret =
			    himax_read_event_stack(ts->client, buf,
						   hx_touch_data->
						   touch_info_size);
		}
		...
		if (!ret)
#else
		if (!himax_read_event_stack
		    (ts->client, buf, hx_touch_data->touch_info_size))
#endif
		{
	 ...
	 /*SMWP*/ case 2:
		...
		if (!himax_read_event_stack
		    (ts->client, buf, hx_touch_data->event_size)) {
			E("%s: can't read data from chip!\n", __func__);
			goto err_workqueue_out;
		}
		break;
...
int himax_checksum_cal(struct himax_ts_data *ts, uint8_t * buf, int ts_status)
{
	...
	if (ts_status == HX_REPORT_COORD) {
		...
		if ((hx_esd_event == length || hx_zero_event == length)
			...
			shaking_ret =
			    himax_ic_esd_recovery(hx_esd_event, hx_zero_event,
						  length);
				   ...
				   unsigned long val, void *v)
{
	...
	if (psy->desc->get_property != NULL) {
		retval =
		    psy->desc->get_property(psy, POWER_SUPPLY_PROP_STATUS,
					    &prop);
	}
	if (retval != 0) {
		return NOTIFY_OK;
	...
	if (status != POWER_SUPPLY_STATUS_DISCHARGING
	    && last_status == POWER_SUPPLY_STATUS_DISCHARGING) {
		last_status = status;
		USB_detect_flag = 1;
		...
		USB_detect_flag = 0;
		   ...
		   && last_status == POWER_SUPPLY_STATUS_UNKNOWN) {
		last_status = status;
		USB_detect_flag = 1;
	} else {
		last_status = status;
			    ...
			    const struct i2c_device_id *id)
{
 ...
 FW_force_upgrade:
	...
	if (ic_data->vendor_touch_cfg_ver < 0x30
		...
		auto_update_flag |= ((ic_data->vendor_fw_ver < g_i_new_FW_VER)
				     || (ic_data->vendor_config_ver <
					 g_i_new_CFG_VER));
	}
#else
	...
	if (ic_data->vendor_touch_cfg_ver == 0xB0) {
		E("ic_data->vendor_touch_cfg_ver = %d",
